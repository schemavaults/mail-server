...
const REASON_COPY: Record<
  NonNullable<ApiKeyRevokedEmailProps["revokedReason"]>,
  { label: string; lede: (keyName: string, product: string) => string }
> = {
  "user-initiated": {
    label: "Revoked by you",
    lede: (keyName, product) =>
      `The API key "${keyName}" was revoked from your ${product} account at your request. Any application using it will start receiving 401 Unauthorized responses immediately.`,
  },
  "admin-initiated": {
    label: "Revoked by an administrator",
    lede: (keyName, product) =>
      `The API key "${keyName}" was revoked from your ${product} account by an account administrator. Any application using it will start receiving 401 Unauthorized responses immediately.`,
  },
  rotated: {
    label: "Rotated",
    lede: (keyName, product) =>
      `The API key "${keyName}" was rotated on your ${product} account. The old credential is now invalid — make sure every deployed service has switched to the replacement key.`,
  },
  expired: {
    label: "Expired",
    lede: (keyName, product) =>
      `The API key "${keyName}" on your ${product} account has reached its expiration date and has been deactivated. Any application using it will start receiving 401 Unauthorized responses immediately.`,
  },
  "suspected-compromise": {
    label: "Revoked for suspected compromise",
    lede: (keyName, product) =>
      `The API key "${keyName}" was revoked from your ${product} account because it was flagged as potentially compromised. We strongly recommend rotating any other credentials that may have been exposed at the same time.`,
  },
};
...
export default function ApiKeyRevokedEmail(
  ...
  const reason: NonNullable<ApiKeyRevokedEmailProps["revokedReason"]> =
    props.revokedReason ?? "user-initiated";
  const reasonCopy = REASON_COPY[reason];
  const isSuspectedCompromise: boolean = reason === "suspected-compromise";
  ...
  const headerGradient: string = isSuspectedCompromise
    ? `linear-gradient(135deg, ${BRAND_RED} 0%, ${BRAND_RED_DARK} 100%)`
    : `linear-gradient(135deg, ${BRAND_BLUE} 0%, ${BRAND_BLUE_DARK} 100%)`;
  const ctaColor: string = isSuspectedCompromise ? BRAND_RED : BRAND_BLUE_DARK;
      ...
      >
        ...
        >
          ...
          <Section style={{ padding: "28px 32px 8px 32px" }}>
            ...
            >
              {reasonCopy.lede(props.keyName, productName)}
            </Text>
          </Section>
          ...
          <Section style={{ padding: "16px 32px 0 32px" }}>
            ...
            >
              ...
              >
                Any service still using this key will start failing with{" "}
                <strong>401 Unauthorized</strong> on its next request. If you
                still need API access, create a replacement key from the API
                keys page and update your deployed environments.
              </Text>
            </div>
          </Section>
          ...
          {isSuspectedCompromise ? (
            <Section style={{ padding: "16px 32px 8px 32px" }}>
              ...
              >
                ...
                >
                  Rotate any credentials that lived in the same environment,
                  review recent activity logs for the affected account, and
                  re-issue a new key only after the source of the leak has
                  been addressed.
                </Text>
              </div>
            </Section>
            ...
            <Section style={{ padding: "16px 32px 8px 32px" }}>
              ...
              >
                ...
                >
                  ...
                  review your account's recent activity, change your password,
                  and contact us at{" "}
                  <a
...
ApiKeyRevokedEmail.PreviewProps = {
  ...
  keyPrefix: "svlts_pk",
  keyLastFour: "a3f2",
  revokedReason: "suspected-compromise",
  revokedAt: "Jun 23, 2026 17:42 UTC",
