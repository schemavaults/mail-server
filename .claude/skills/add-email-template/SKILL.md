...
- Resolve optional props with `typeof x === "string" && x.length > 0 ? x : defaultValue` — brand-related defaults (product name, main URL, support email, accent colors) MUST come from `getEmailBrand()` (relative import from `./brand`) so templates stay white-labellable: call `const brand = getEmailBrand()` inside the component and derive `const BRAND_BLUE = brand.colors.accent; const BRAND_BLUE_DARK = brand.colors.accentDark;` there. Keep neutral palette constants (`FOREGROUND`, `BORDER`, etc.) and semantic status colors inline as module-level hex constants, since email clients don't resolve CSS custom properties. Never hardcode a brand name, brand URL, or brand support email in template copy or defaults
- Attach `<Pascal>Email.PreviewProps = { ... } satisfies <Pascal>EmailProps` at the bottom of the file. Omit optional brand-identity props (productName, supportEmail, etc.) so previews render with the configured brand defaults, and use neutral `example.com` URLs for required sample links. **This is the single source of truth for sample props** — the `bun run dev:mail` preview, the `/admin/templates` iframe preview, and the `validateProps` unit test all read from `.PreviewProps` via `src/lib/EmailTemplatesCatalog/sampleProps.ts`. Any payload you put here MUST also pass the catalog entry's `validateProps` (the unit test enforces this in CI)
...
- `public async renderPlainTextVersion(props)` — hand-rolled plain-text fallback (array of lines, `.join("\n")`). Apply the same optional-prop defaulting you do in the component, sourcing brand defaults from `getEmailBrand()` (import from `@/email-templates/brand`)
